...
class PhotoController extends Controller
{
    ...
    public function accessRules()
    {
        return array(
            ...
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                'actions' => array('create', 'update', 'placescombo'),
                'users' => array('@'),
            ),
    ...
    public function actionPlacescombo()
    {
        if (isset($_POST['Photo'])) {
            $tours_id = $_POST['Photo']['tours_id']; // AQUI NO TRAE NADA CUANDO SELECCIONO EL PRIMER COMBO.
            $data = Place::model()->findAll('tours_id=:tours_id',
                array(':tours_id' => $tours_id));

            $data = CHtml::listData($data, 'id', 'name');
            foreach ($data as $value => $place) {
                echo CHtml::tag('option',
                    array('value' => $value), CHtml::encode($place), true);
            }
        }
...
//            Yii::app()->cache->set('tours', $arrayTours,50);
//        }
